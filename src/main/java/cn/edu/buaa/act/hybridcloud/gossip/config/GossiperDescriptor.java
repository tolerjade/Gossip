...
import cn.edu.buaa.act.hybridcloud.gossip.locator.SeedProvider;
import cn.edu.buaa.act.hybridcloud.gossip.locator.SimpleSeedProvider;
...
public class GossiperDescriptor {
    private static Logger logger = Logger.getLogger(GossiperDescriptor.class);
    ...
    private static InetSocketAddress master;
    private static InetSocketAddress clientListen;
    ...
    public static InetSocketAddress getMaster() {
        checkHasInited();
        return master;
    }
    ...
    public static InetSocketAddress getClientListen(){
        checkHasInited();
        return clientListen;
    }
    ...
    private static void parseConfigInfo() {
    	...
    	try {
            if (conf.getListen_address() != null)
            {
                ...
                try
                {
                    master = InetSocketAddressUtil.parseInetSocketAddress(conf.getMaster());
                    listenAddress =  InetSocketAddressUtil.parseInetSocketAddress(conf.getListen_address());
                    clientListen = InetSocketAddressUtil.parseInetSocketAddress(conf.getListen_client());
                }
