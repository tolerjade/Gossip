package cn.edu.buaa.act.hybridcloud.gossip.net;
...
import cn.edu.buaa.act.hybridcloud.gossip.net.MessageVerb.Verb;
...
public class MessagingService{
    ...
    private static final Logger logger_ = Logger.getLogger(MessagingService.class);
    ...
    private List<SyncThread> syncThreads = Lists.newArrayList();
...
//      MessagingService.instance().registerVerbHandlers(Verb.INTERNAL_RESPONSE, new ResponseVerbHandler());
  	...
  	verbHandlers_.put(Verb.LOGS_DIGEST_SYN, new LogsDigestSynVerbHandler());
  	verbHandlers_.put(Verb.LOGS_DIGEST_ACK, new LogsDigestAckHandler());
  	verbHandlers_.put(Verb.LOGS_DIGEST_ACK2, new LogsDigestAck2Handler());
  	verbHandlers_.put(Verb.LOG_REQUIRE_TOSN, new LogRequireTOSNHandler());
  	verbHandlers_.put(Verb.LOG_RESPONSE_TOSN, new LogResponseTOSNHandler());
...
//        }
    ...
    public void listenClient(InetSocketAddress inetSocketAddress) throws IOException{
        ServerSocket socket = new ServerSocket(inetSocketAddress.getPort(),5,inetSocketAddress.getAddress());
        System.out.println("listen inetSocketAddress:"+inetSocketAddress);

        SyncThread th = new SyncThread(socket, "ACCEPT-" + inetSocketAddress);
        th.start();
        syncThreads.add(th);
    }
    /**
...
//        listenGate.signalAll();
    ...
    private static class SyncThread extends Thread{
        ...
        private final List<ClientService> socketList = new ArrayList();

        SyncThread(ServerSocket server, String name){
            super(name);
            this.server = server;
        }
        ...
        {
            server.close();
            for (ClientService socket : socketList) {
                socket.close();
            }
        }
